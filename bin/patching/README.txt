...
we have added "projection_parent_gene", which specifies the Ensembl gene model id for the corresponding
gene in the reference genome (C57BL/6J), and "mgp", which specifies the original Mouse Genomes Project
ids from earlier releases.

Background/motivation: Prior to Ensembl release-114, the mouse strain genome annotations (GFF3 files) 
contained projection_parent_gene attributes, which we used to link strain gene models with the canonical
gene from MGI and with corresponding gene models in other strains. For example, the A/J gene
ID=MGP_AJ_G0026191, had projection_parent_gene=ENSMUSG00000027168, which we used to link this gene with
MGI:97490 (Pax6) and with equivalent genes in other strains, e.g., ID=MGP_CBAJ_G0025928 in CBA/J, which
also has projection_parent_gene=ENSMUSG00000027168.
...
(2) Gene models were assigned all new IDs. A/J gene MGP_AJ_G0026191 is now ENSMUSG00195028435. 
...
able to recover the prior MGP ids and when successful, we add them as a new attribute "mgp". The patched
files are being made public in case others might also find them useful.
...
Complication:
In the majority of cases, the associations between strain genes, projection parents, and MGP ids is
nicely 1:1.  But not aways. Multiple strain genes in a given genome may have the same projection parent.
This was true before release 114 and is true after. The bottom line is that both projection_parent_gene
and mgp attributes can be multivalued.
...
within a single genome that refer to the same projection parent gene. The MGI sequence group discussed this and 
decided to cap (at 3) the max number of strain genes (in one genome) that can be associated with 
one projection parent. This limit was set because cases involving many strain genes for the same projection 
parent gene in the same strain are caused by repetitive sequences within the strain gene, such as endogenous 
retrovirus (ERV) elements.  If there are more than the limit, then none of the strain gene is associated with 
the projection parent. 

3. Old MGP ids (adding "mgp=").
The patching process uses a snapshot of data from MGI (prior to the Ensembl 116 update) to recover the 
MGPs for many genes.  The archived data is a table, one row per strain gene, with columns that include
the MGP id and the Ensembl id (projection parent) of the associated MGI gene. The Ensembl id is 
